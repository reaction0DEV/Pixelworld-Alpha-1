...
/* ══════════════════════════════════════════════════════════
   SYSTÈME DE LOGS
   info.log     → connexions, rooms, sauvegardes, erreurs
   absolute.log → absolument tout (pixels, debug, etc.)
══════════════════════════════════════════════════════════ */
const _logPath  = require('path');
const _logFs    = require('fs');
const _LOG_DIR  = _logPath.join(__dirname, 'logs');
const _INFO     = _logPath.join(_LOG_DIR, 'info.log');
const _ABSOLUTE = _logPath.join(_LOG_DIR, 'absolute.log');
const _MAX_SIZE = 10 * 1024 * 1024; // 10 MB → rotation

if (!_logFs.existsSync(_LOG_DIR)) _logFs.mkdirSync(_LOG_DIR, { recursive: true });

function _rotate(f) {
  try { if (_logFs.statSync(f).size >= _MAX_SIZE) _logFs.renameSync(f, f.replace('.log','_'+Date.now()+'.log')); } catch {}
}
function _write(f, level, cat, msg) {
  const line = `[${new Date().toISOString()}] [${level.padEnd(5)}] [${cat.padEnd(10)}] ${msg}\n`;
  _rotate(f);
  try { _logFs.appendFileSync(f, line); } catch {}
}

// File d'attente pour les debug (évite les écritures disque en rafale)
let _debugQueue = [];
setInterval(() => {
  if (_debugQueue.length === 0) return;
  const lines = _debugQueue.join('');
  _debugQueue = [];
  _rotate(_ABSOLUTE);
  try { _logFs.appendFileSync(_ABSOLUTE, lines); } catch {}
}, 2000); // flush toutes les 2s

const Log = {
  info (cat, msg) { const t=`[INFO ] [${cat}] ${msg}`; console.log(t);   _write(_INFO,'INFO', cat, msg); _write(_ABSOLUTE,'INFO', cat, msg); },
  warn (cat, msg) { const t=`[WARN ] [${cat}] ${msg}`; console.warn(t);  _write(_INFO,'WARN', cat, msg); _write(_ABSOLUTE,'WARN', cat, msg); },
  error(cat, msg) { const t=`[ERROR] [${cat}] ${msg}`; console.error(t); _write(_INFO,'ERROR',cat, msg); _write(_ABSOLUTE,'ERROR',cat, msg); },
  // debug → mise en file, écriture groupée toutes les 2s (pas de blocage I/O)
  debug(cat, msg) {
    _debugQueue.push(`[${new Date().toISOString()}] [DEBUG] [${cat.padEnd(10)}] ${msg}\n`);
  },
};
/* ══════════════════════════════════════════════════════════ */
...
const SECURITY = {
  MAX_PIXELS_PER_SEC: 50000, MAX_BATCH_SIZE: 10000,
...
/* ── LOAD / SAVE ROOMS ── */
function ensureGeneralRooms() {
  const generals = [
    { id: 'GENERAL1', name: '🌍 Général 1' },
    { id: 'GENERAL2', name: '🌍 Général 2' },
    { id: 'GENERAL3', name: '🌍 Général 3' },
  ];
  for (const g of generals) {
    if (!rooms.has(g.id)) {
      rooms.set(g.id, {
        id: g.id, name: g.name, owner: 'system', ownerId: null,
        ownerDiscordId: null, passwordHash: null, maxPlayers: 200,
        canvas: {}, bannedSet: new Set(), mods: [],
        createdAt: Date.now(), canvasW: 800, canvasH: 600,
        isGeneral: true, protectedZones: [], reports: {},
        chatLog: [], antiSpam: false, members: new Map(),
      });
      Log.info('BOOT', `Room ${g.id} recréée (manquante)`);
    }
  }
}
...
function loadRooms() {
  if (!fs.existsSync(ROOMS_FILE)) {
    Log.warn('BOOT', `${ROOMS_FILE} introuvable — démarrage sans rooms`);
    return;
  }
  try {
    const raw = fs.readFileSync(ROOMS_FILE, 'utf8');
    if (!raw || raw.trim() === '') {
      Log.warn('BOOT', 'rooms.json est vide — démarrage sans rooms');
      return;
    }
    const parsed = JSON.parse(raw);
    parsed.forEach(r => {
      r.members      = new Map();
      r.bannedSet    = new Set(r.banned || []);
      r.isGeneral    = r.isGeneral    || false;
      r.protectedZones = r.protectedZones || [];
      r.reports      = r.reports      || {};
      r.chatLog      = r.chatLog      || [];
      r.antiSpam     = r.antiSpam     || false;
      r.canvas       = r.canvas       || {};
      const pixCount = Object.keys(r.canvas).length;
      Log.info('BOOT', `Room chargée: ${r.name} (${r.id}) — ${pixCount} pixel(s)`);
      rooms.set(r.id, r);
    });
    Log.info('BOOT', `${rooms.size} room(s) chargée(s) au total`);
  } catch(e) {
    Log.error('BOOT', `Erreur lecture rooms.json: ${e.message}`);
    // Tentative de récupération depuis le backup .tmp
    const tmpFile = ROOMS_FILE + '.tmp';
    if (fs.existsSync(tmpFile)) {
      Log.warn('BOOT', 'Tentative de récupération depuis rooms.json.tmp...');
      try {
        const raw2 = fs.readFileSync(tmpFile, 'utf8');
        JSON.parse(raw2).forEach(r => {
          ...
          r.antiSpam = r.antiSpam || false; r.canvas = r.canvas || {};
          rooms.set(r.id, r);
        });
        Log.info('BOOT', `Récupération réussie: ${rooms.size} room(s)`);
      } catch(e2) {
        Log.error('BOOT', `Récupération échouée aussi: ${e2.message}`);
      }
    }
  }
}
function saveRooms() {
  ...
  try {
    ...
    Log.error('SAVE', `Erreur écriture: ${e.message}`);
...
function scheduleSave(roomId) {
  ...
  saveDebounce.set(roomId, setTimeout(() => {
    ...
    Log.info('SAVE', `Room ${roomId} sauvegardée (debounce)`);
  }, 1000));
}
...
setInterval(() => {
  saveRooms();
  Log.info('SAVE', `Sauvegarde complète 90s — ${rooms.size} room(s)`);
}, 90 * 1000);
...
function createDefaultRooms(){
  ...
  if(c>0){saveRooms();Log.info('BOOT',`${c} lobby(s) créé(s)`);}
}
createDefaultRooms();
...
app.get('/auth/discord/callback', async (req, res) => {
  ...
  try {
    ...
    Log.info('DISCORD', `${user.username} (${user.id}) connecté`);
    ...
    Log.error('DISCORD', `OAuth error: ${err.message}`);
...
function requireAdmin(req, res, next) {
  ...
  // Accepter aussi le cookie de session
  const cookieToken = req.cookies?.pw_session;
  const session = discordSessions.get(token) || discordSessions.get(cookieToken);
...
io.on('connection', socket => {
  ...
  Log.info('SOCKET', `Connexion ${socket.id}`);
  ...
  socket.on('room:join', async(data,cb)=>{
    ...
    Log.info('ROOM',`${pseudo} a rejoint ${roomId}`); Log.debug('ROOM',`Socket ${socket.id} → ${roomId}`);
    cb({ok:true});
    // Annuler la suppression si quelqu'un rejoint avant les 30s
    if (room._deleteTimer) { clearTimeout(room._deleteTimer); room._deleteTimer = null; Log.info('ROOM', `Suppression annulée pour ${roomId} — joueur rejoint`); }
  ...
  socket.on('pixel:set',(data)=>{
    ...
    // Tronquer au lieu de rejeter — on traite les MAX_BATCH_SIZE premiers pixels
    const pixelSlice = pixels.slice(0, SECURITY.MAX_BATCH_SIZE);
    if(!checkPixelRate(socket.id,pixelSlice.length))return;
    const changes=[];
    for(const p of pixelSlice){
      const {x,y,color}=p;
    ...
    if(changes.length){
      ...
      Log.debug('PIXEL',`${info.pseudo} → ${changes.length} pixel(s) dans ${info.roomId}`);
      // Si suppression de pixel → sauvegarde immédiate (pas de debounce)
      const hasDeletion = changes.some(p => p.color === null);
      if (hasDeletion) {
        saveRooms();
      } else {
        // Sinon debounce 1s pour les ajouts (évite les écritures disque en rafale)
  ...
  socket.on('room:delete',(cb)=>{
    ...
    Log.info('ROOM',`Supprimée manuellement: ${info.roomId} par ${info.pseudo}`); rooms.delete(info.roomId); recentLeft.delete(info.roomId); saveRooms(); broadcastRoomList(); cb?.({ok:true});
  });
  ...
  socket.on('disconnect',()=>{ Log.info('SOCKET', `Déconnexion ${socket.id}`); handleLeave(socket); sockets.delete(socket.id); });
});
...
function handleLeave(socket) {
  ...
  // Annuler tout debounce en cours pour cette room (évite écrasement)
  if (saveDebounce.has(info.roomId)) {
    clearTimeout(saveDebounce.get(info.roomId));
    saveDebounce.delete(info.roomId);
  }
  // Sauvegarde immédiate avant de traiter le départ — garantit que rien n'est perdu
  saveRooms();
  ...
  if (room.members.size === 0 && !room.isGeneral) {
    // Attendre 30s avant de supprimer — si quelqu'un rejoint entre temps, annuler
    saveRooms();
    broadcastRoomList();
    Log.info('ROOM', `Room ${info.roomId} vide — suppression dans 30s`);
    const deleteTimer = setTimeout(() => {
      const r = rooms.get(info.roomId);
      if (r && r.members.size === 0 && !r.isGeneral) {
        rooms.delete(info.roomId);
        recentLeft.delete(info.roomId);
        saveRooms();
        broadcastRoomList();
        Log.info('ROOM', `Room ${info.roomId} supprimée après 30s vide`);
      }
    }, 30 * 1000);
    // Annuler si quelqu'un rejoint avant les 30s
    room._deleteTimer = deleteTimer;
  }
}
...
server.listen(PORT,()=>Log.info('BOOT',`PixelWorld démarré → http://localhost:${PORT}`));
