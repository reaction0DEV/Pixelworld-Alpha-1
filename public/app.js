...
let lastDrawX = -1, lastDrawY = -1; // interpolation pinceau
...
// Canvas offscreen pour le rendu des pixels (évite de tout redessiner à chaque frame)
let offscreen = null, offCtx = null, offW = 0, offH = 0, offDirty = true;
function getOffscreen(w, h) {
  if (!offscreen || offW !== w || offH !== h) {
    offscreen = document.createElement('canvas');
    offscreen.width = w; offscreen.height = h;
    offCtx = offscreen.getContext('2d');
    offW = w; offH = h; offDirty = true;
  }
  return offCtx;
}
...
async function initAuth() {
  ...
  // 1. Cookie prioritaire — si valide, on s'arrête là, on ignore tout le reste
  try {
    const cookieRes = await fetch('/auth/current-session');
    const cookieData = await cookieRes.json();
    if (cookieData.valid) {
      sessionToken = '__cookie__';
      // Nettoyer l'URL et le localStorage pour éviter toute confusion
  ...
  // 2. Pas de cookie valide — essayer le token URL ou localStorage
...
function onDiscordLogin(session) {
  ...
  // Pas de validation socket — le cookie est validé côté serveur via /auth/current-session
  // et le token URL est validé via /auth/session avant d'appeler onDiscordLogin
}
...
socket.on('room:joined', (data) => {
  ...
  mmDirty = true;
  showGame();
  // setTimeout 150ms : laisser le temps au DOM de stabiliser avant de dessiner
  setTimeout(() => {
    render();
    updateMinimap(true);
  }, 150);
...
socket.on('pixel:batch', (changes) => {
  ...
  mmDirty = true; offDirty = true; scheduleRender();
});
...
function sendPendingPixels() {
  ...
  // Envoyer en chunks de 2000 pixels avec délai pour éviter le lag
  const CHUNK = 2000;
  for (let i = 0; i < batch.length; i += CHUNK) {
    const slice = batch.slice(i, i + CHUNK);
    if (i === 0) {
      socket.emit('pixel:set', { pixels: slice });
    } else {
      setTimeout(() => socket.emit('pixel:set', { pixels: slice }), (i / CHUNK) * 50);
    }
  }
  pendingPixels = {};
}
function startRealtimeFlush() { if (_flushTimer) return; _flushTimer = setInterval(() => { if (Object.keys(pendingPixels).length) sendPendingPixels(); }, 150); }
...
// Cache de couleurs hex → [r,g,b] pour éviter le parsing répété
const _colorCache = new Map();
function _hexToRgb(hex) {
  if (_colorCache.has(hex)) return _colorCache.get(hex);
  const r = parseInt(hex.slice(1,3),16), g = parseInt(hex.slice(3,5),16), b = parseInt(hex.slice(5,7),16);
  const rgb = [r,g,b]; _colorCache.set(hex, rgb); return rgb;
}

let _lastCamX = null, _lastCamY = null, _lastZoom = null, _lastTheme = null;
function render() {
  ...
  const psi = Math.max(1, Math.round(ps));

  // Fond
  ctx.fillStyle = isDark ? '#111120' : '#ffffff';
  ctx.fillRect(0, 0, W, H);

  // Grouper les pixels par couleur pour minimiser les changements de fillStyle
  ...
  for (const [k, px] of pixels) {
    ...
    if (!byColor.has(px.color)) byColor.set(px.color, []);
    byColor.get(px.color).push([wx, wy]);
  }
  for (const [color, pts] of byColor) {
    ctx.fillStyle = color;
    for (const [wx, wy] of pts) {
      const sx = Math.round((wx - camX) * ps), sy = Math.round((wy - camY) * ps);
      ctx.fillRect(sx, sy, psi, psi);
    }
  }
...
vp.addEventListener('mousedown', e => {
  ...
  drawing = true; stroke = {}; lastDrawX = x; lastDrawY = y; startRealtimeFlush(); paintBrush(x, y, tool === 'erase' ? null : col); render();
});

vp.addEventListener('mousemove', e => {
  const { x, y } = vpCoords(e);
  if (x >= 0 && x < WORLD_W && y >= 0 && y < WORLD_H) document.getElementById('gh-xy').textContent = `x:${x}  y:${y}  [${WORLD_W}×${WORLD_H}]`;
  if (panning) { const ps = PIXEL_SIZE * zoom; camX = panCamX - (e.clientX - panStartX) / ps; camY = panCamY - (e.clientY - panStartY) / ps; clampCam(); scheduleRender(); return; }
  ...
  if (!drawing) return;
  // Interpolation Bresenham : combler les trous entre deux positions
  if (lastDrawX >= 0) {
    for (const [px, py] of linePts(lastDrawX, lastDrawY, x, y)) {
      paintBrush(px, py, tool === 'erase' ? null : col);
    }
  } else {
    paintBrush(x, y, tool === 'erase' ? null : col);
  }
  lastDrawX = x; lastDrawY = y;
  scheduleRender(); // rAF — max 60fps peu importe la vitesse du curseur
});
...
vp.addEventListener('mouseup', e => {
  ...
  if (drawing) { stopRealtimeFlush(); commit(); drawing = false; lastDrawX = -1; lastDrawY = -1; }
});
...
vp.addEventListener('touchstart', e => { e.preventDefault(); if (e.touches.length === 2) { panning = true; lastTouches = e.touches; return; } const t = e.touches[0], r = vp.getBoundingClientRect(); const { x, y } = screenToWorld(t.clientX - r.left, t.clientY - r.top); drawing = true; stroke = {}; lastDrawX = x; lastDrawY = y; startRealtimeFlush(); paintBrush(x, y, tool === 'erase' ? null : col); render(); }, { passive: false });
vp.addEventListener('touchmove', e => { e.preventDefault(); if (e.touches.length === 2 && panning && lastTouches) { const dx = (e.touches[0].clientX + e.touches[1].clientX)/2 - (lastTouches[0].clientX + lastTouches[1].clientX)/2, dy = (e.touches[0].clientY + e.touches[1].clientY)/2 - (lastTouches[0].clientY + lastTouches[1].clientY)/2; camX -= dx / (PIXEL_SIZE * zoom); camY -= dy / (PIXEL_SIZE * zoom); lastTouches = e.touches; clampCam(); render(); return; } if (!drawing) return; const t = e.touches[0], r = vp.getBoundingClientRect(); const { x, y } = screenToWorld(t.clientX - r.left, t.clientY - r.top);
  if (lastDrawX >= 0) { for (const [px, py] of linePts(lastDrawX, lastDrawY, x, y)) { paintBrush(px, py, tool === 'erase' ? null : col); } } else { paintBrush(x, y, tool === 'erase' ? null : col); }
  lastDrawX = x; lastDrawY = y; scheduleRender(); }, { passive: false });
vp.addEventListener('touchend', () => { panning = false; lastTouches = null; if (drawing) { stopRealtimeFlush(); commit(); drawing = false; lastDrawX = -1; lastDrawY = -1; } });
